...
Collision handling uses transformed world-space axis-aligned bounding boxes and declaration order. Overlapping primitives inside the same top-level named component are grouped and assigned a `union-*` identifier, so locally authored compound parts retain the default-union behavior. A later collision outside that component space is packed to the closest free face-aligned coordinate around the earlier objects instead of being unioned; the component's combined bounds determine that position, and every part receives the same translation. Equidistant placements use the stable `+X`, `-X`, `+Y`, `-Y`, `+Z`, `-Z` order. Explicit boolean tools retain their authored offset from the base selected by declaration-order CSG targeting and follow that base when the expression is packed. Rendering applies a subtle union highlight to default-union groups.
